...
buildbuild is a webhook-free CI system for Nix-based C++ projects. It polls GitHub's Events API instead of using webhooks, builds 8 variants (release, debug, asan, ubsan, tsan, msan, coverage, fuzz), and stores artifacts as uncompressed NAR files in a Git-based binary cache for delta compression.
...
- **PULL** — Nix flake that defines all 8 build variants. Shell scripts orchestrate: update source → build → export NAR → upload cache → update status. Config in `PULL/cache/config.json`.
